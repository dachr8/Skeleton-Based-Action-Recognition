...
def ntu_transform(raw_data):
    ...
    for raw_ctvm in raw_data:
        ...
        for i in range(raw_ctvm.shape[3]):
            transform_mctv.append(view_invariant_transform(raw_ctvm[:, :, :, i]))
...
if __name__ == '__main__':
    data = np.load('../data/ntu_raw/xsub/val_data_joint.npy')
    data = ntu_transform(data)
    np.save('../data/ntu/xsub/val_data_vit.npy', data)

    data = np.load('../data/ntu_raw/xsub/train_data_joint.npy')
    data = ntu_transform(data)
    np.save('../data/ntu/xsub/train_data_vit.npy', data)
